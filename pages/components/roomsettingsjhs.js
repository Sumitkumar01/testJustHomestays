...
export default function SRooms(props) {
  ...
  useEffect(() => {
    ...
    const updatedImages = props?.images?.map((image) => ({
      ...image,
      path: "https://test.justhomestay.in/" + image.path,
    }));
  ...
  const handleSubmit = async () => {
    ...
    if (createRoom.status === 200) {
      const options2 = {
        method: "POST",
        url: `${process.env.API_URL}Bedrooms/${createRoom.data.ncRecordId}/mm/JHSList/${pid}`,
        headers: {
          "xc-token": `${process.env.API_KEY}`,
        },
      };
          ...
          {existing?.length != maxRooms ? (
            <div className="flex flex-col gap-y-2 py-2">
              ...
              <div className="flex gap-x-2">
                ...
                {selectedImage ? (
                  <Image width={200} height={200} src={selectedImage} />
                ) : null}
              </div>
              <input
                type="number"
