...
import AddAmen from "../components/AmenAdd";
...
export default function Amenities() {
  ...
  const getData = (options) => {
    axios.request(options).then(function (res) {
      setExisting(res.data.list[0].Amenities);
      setData(res.data.list[0].Amenity)
    });
  };
  ...
  if (isLoggedIn) {
    ...
    return (
      <div className="flex w-[80%] flex-col justify-center items-center mx-auto ">
        <AddAmen data={data} existing={existing} logout={logOutHandler} />
      </div>
    );
  } else {
    return (
